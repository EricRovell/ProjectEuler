...
# solving the problem
def digitalsum():
  # getting rid of perfect squares
  numbers = {num for num in range(2, 100)} - {num ** 2 for num in range(1, 10)}
  ...
  for number in numbers:
    number = list(digits(number))
    number.reverse()
    sqroot = sqrt(number, [0], 100)
    total += sum((sqroot['integral'] + sqroot['fractional'])[:100])
